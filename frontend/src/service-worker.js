...
// Cache the static assets with a cache-first strategy
registerRoute(
  /\.(?:js|css)$/,
  new StaleWhileRevalidate({
...
// For example, you could add support for background sync:
/*
self.addEventListener('sync', (event) => {
  if (event.tag === 'submitRoofRequest') {
    event.waitUntil(submitPendingRequests());
  }
});

async function submitPendingRequests() {
  ...
  for (const request of pendingRequests) {
    try {
      ...
      if (response.ok) {
        ...
        self.registration.showNotification('RoofAI', {
          body: 'Your roof estimate request has been submitted successfully!',
          icon: '/logo192.png'
        });
      }
    } catch (error) {
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('RoofAI', 1);
